...
const speakCaster = (text, langStr) => {
  ...
  if (langStr === 'en') {
      ...
      utterance.voice = voices.find(v => v.lang.startsWith('vi')) || null;
...
function App() {
  ...
  const [state, setState] = useState({ status: 'waiting', players: [], logs: [], timer: 0, config: {title:'ARENA', bg:'#0f172a', w:800, h:600, lang:'vi'} });
  ...
  // Tính trạng thái để CSS Khóa cuộn màn hình
          ...
          if (!hasAnnouncedWin.current) {
              ...
              if (soundEnabled) {
                  ...
                  const summary = lang === 'en' 
                    ...
                    : `Trận đấu kết thúc sau ${duration} giây! Chúc mừng ${winner.name}. Cầm ${winner.weapon} và ${winner.shield}, huyền thoại này đã quét sạch ${kills} mạng!`;
  ...
  return (
    // Sử dụng h-screen và overflow-hidden khi Play để khóa cuộn, full màn hình tuyệt đối
    <div className={`bg-slate-950 text-slate-100 p-2 md:p-4 font-sans flex flex-col items-center overflow-hidden ${isPlaying ? 'h-screen' : 'min-h-screen'}`}>
      ...
      {/* HEADER TỐI ƯU KHÔNG GIAN KHI CHƠI */}
      <div className={`flex flex-col md:flex-row items-center gap-2 md:gap-4 w-full justify-between transition-all ${isPlaying ? 'mb-2 px-2 max-w-full' : 'mb-4 md:mb-8 max-w-7xl'}`}>
        ...
        <button className={`text-xs p-2 font-bold rounded-lg transition-all border ${soundEnabled ? 'bg-green-900 border-green-400 text-green-100' : 'bg-slate-800 border-slate-700 text-slate-400'}`} onClick={() => {
            ...
            if(!soundEnabled) { playSFX('slash'); speakCaster(state.config?.lang === 'en' ? "Caster AI is ready!" : "Bình luận viên đã sẵn sàng!", state.config?.lang); } 
            else { if(window.speechSynthesis) window.speechSynthesis.cancel(); }
        }}>
            {soundEnabled ? "🔊 AUDIO: ON" : "🔇 AUDIO: OFF (CLICK TO UNMUTE)"}
        </button>
      </div>

      {/* CÁC PANEL QUẢN LÝ LOBBY (Chỉ hiện khi chưa Play) */}
      {isHost && !isPlaying && state.config && (
        <div className="mb-6 p-3 md:p-4 bg-slate-900 border border-yellow-600/30 rounded-xl grid grid-cols-2 md:grid-cols-5 gap-3 w-full max-w-7xl text-xs md:text-sm shadow-xl">
          ...
          <select className="bg-slate-800 p-2 rounded outline-none font-bold" value={state.config.lang} onChange={e=>call('/config', {...state.config, lang: e.target.value})}>
      ...
      {state.status === 'strategy' && (
        <div className="max-w-2xl w-full bg-slate-900 p-5 md:p-10 rounded-3xl border-2 border-purple-500/50 shadow-2xl mt-10">
          <div className="text-center mb-6">
            ...
            {isHost && <button className="mt-4 bg-red-600 hover:bg-red-500 px-6 py-2 rounded-full font-black text-white shadow-lg animate-pulse" onClick={()=>call('/start-now')}>🚀 START NOW (RANDOM OTHERS)</button>}
          </div>
      ...
      {/* PHASE 3: PLAYING FULL SCREEN LAYOUT */}
      {isPlaying && state.config && (
        <div className="flex flex-col lg:flex-row gap-4 w-full max-w-full flex-grow overflow-hidden pb-2 px-2">
          ...
          {/* VÙNG MAP: TỰ ĐỘNG SCALE ĐẦY ĐỦ KHÔNG GIAN BẰNG OBJECT-CONTAIN */}
          ...
          {/* VÙNG BÌNH LUẬN CỐ ĐỊNH BÊN PHẢI */}
          <div className="bg-slate-900 p-4 rounded-2xl w-full lg:w-96 flex-shrink-0 flex flex-col border border-slate-800 shadow-xl overflow-hidden h-[30vh] lg:h-full">
             ...
             <div className="flex-grow overflow-y-auto space-y-2 flex flex-col-reverse font-mono text-[10px] md:text-xs leading-relaxed pr-1 custom-scrollbar">
                {[...(state.logs || [])].reverse().map((l, i) => <div key={i} className={`p-2 md:p-3 rounded-lg border-l-4 ${l.includes('💀') || l.includes('🏆') || l.includes('🔥') ? 'bg-red-950/40 border-red-600 text-red-200' : 'bg-slate-800/80 border-slate-600 text-slate-300'}`}>{l}</div>)}
             </div>
          </div>
