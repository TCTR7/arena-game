...
const playSFX = (type) => {
  ...
  if (type === 'slash') { osc.type = 'sawtooth'; osc.frequency.setValueAtTime(400, now); osc.frequency.exponentialRampToValueAtTime(100, now + 0.1); gain.gain.setValueAtTime(0.1, now); gain.gain.exponentialRampToValueAtTime(0.01, now + 0.1); osc.start(now); osc.stop(now + 0.1); } 
  else if (type === 'arrow') { osc.type = 'sine'; osc.frequency.setValueAtTime(800, now); osc.frequency.linearRampToValueAtTime(200, now + 0.15); gain.gain.setValueAtTime(0.1, now); gain.gain.linearRampToValueAtTime(0.01, now + 0.15); osc.start(now); osc.stop(now + 0.15); }
  else if (type === 'thrust') { osc.type = 'triangle'; osc.frequency.setValueAtTime(300, now); gain.gain.setValueAtTime(0.15, now); gain.gain.exponentialRampToValueAtTime(0.01, now + 0.05); osc.start(now); osc.stop(now + 0.05); }
  else if (type === 'magic') { osc.type = 'square'; osc.frequency.setValueAtTime(600, now); osc.frequency.setValueAtTime(1200, now + 0.1); gain.gain.setValueAtTime(0.05, now); gain.gain.linearRampToValueAtTime(0, now + 0.2); osc.start(now); osc.stop(now + 0.2); }
  else if (type === 'bash') { osc.type = 'square'; osc.frequency.setValueAtTime(100, now); osc.frequency.exponentialRampToValueAtTime(40, now + 0.2); gain.gain.setValueAtTime(0.3, now); gain.gain.exponentialRampToValueAtTime(0.01, now + 0.2); osc.start(now); osc.stop(now + 0.2); }
...
function App() {
  ...
  const draw = (data) => {
    ...
    data.particles?.forEach(p => {
      if(soundEnabled) playSFX(p.type);
    ...
    data.players?.forEach(p => {
      ...
      ctx.font = '22px Arial'; ctx.textAlign = 'center'; ctx.fillStyle = 'white'; ctx.fillText(`${p.icon}${p.s_icon}`, p.x, p.y+10);
      ctx.font = 'bold 10px sans-serif'; ctx.fillText(p.name, p.x, p.y-15);
      
      // HP BAR SCALE TO 300
      ctx.fillStyle = '#444'; ctx.fillRect(p.x-20, p.y+15, 40, 5);
      ctx.fillStyle = p.hp > 150 ? '#22c55e' : p.hp > 60 ? '#f59e0b' : '#ef4444';
      ctx.fillRect(p.x-20, p.y+15, Math.max(0, (p.hp/300)*40), 5);
    });
  };
  ...
  const promptText = `Làm bình luận viên eSports siêu cháy. Tóm tắt trận đấu theo Logs:\n${state.logs.join('\n')}\nWINNER: ${state.winner_info?.name} (VK: ${state.winner_info?.weapon}, Khiên: ${state.winner_info?.shield}, Target AI: ${state.winner_info?.strat?.target_rule}, Núp AI: ${state.winner_info?.strat?.camp_until})`;
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 p-2 md:p-6 font-sans flex flex-col items-center overflow-x-hidden">
      <div className="flex flex-col md:flex-row items-center gap-4 mb-4 md:mb-8 text-center w-full justify-between max-w-7xl">
        ...
        <button className="text-xs bg-slate-800 p-2 rounded border border-slate-700" onClick={() => {setSoundEnabled(!soundEnabled); if(!soundEnabled) playSFX('slash');}}>
            {soundEnabled ? "🔊 Bật Âm Thanh" : "🔇 Tắt Âm Thanh"}
        </button>
      </div>

      {isHost && state.status === 'waiting' && state.config && (
        <div className="mb-6 p-3 md:p-4 bg-slate-900 border border-yellow-600/30 rounded-xl grid grid-cols-2 md:grid-cols-4 gap-3 w-full max-w-6xl text-xs md:text-sm shadow-xl">
          <input className="bg-slate-800 p-2 rounded outline-none" placeholder="Title" value={state.config.title} onChange={e=>call('/config', {...state.config, title: e.target.value})} />
          ...
          <div className="flex items-center gap-2">W:<input className="bg-slate-800 p-2 rounded w-full outline-none" type="number" value={state.config.w} onChange={e=>call('/config', {...state.config, w: parseInt(e.target.value)})} /></div>
          <div className="flex items-center gap-2">H:<input className="bg-slate-800 p-2 rounded w-full outline-none" type="number" value={state.config.h} onChange={e=>call('/config', {...state.config, h: parseInt(e.target.value)})} /></div>
        </div>
      )}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="bg-slate-900 p-5 md:p-8 rounded-2xl border border-slate-800 shadow-2xl">
              ...
              <div className="grid gap-4">
                ...
                <button className="mt-2 bg-blue-600 hover:bg-blue-500 font-bold p-4 rounded-xl shadow-lg transition-transform active:scale-95 text-white" onClick={()=>call('/register', reg).then(d=>{ if(d.status==='success') setReg({...reg, name:'', password:''}); else alert(d.message); })}>GIA NHẬP SẢNH</button>
              </div>
            </div>
            ...
            <div className="bg-slate-900 p-5 md:p-8 rounded-2xl border border-slate-800 flex flex-col shadow-2xl">
              <div className="flex justify-between items-center mb-4 md:mb-6">
                ...
                {isHost && <div className="flex gap-2">
                    ...
                    <button className="bg-red-900/50 text-red-300 hover:bg-red-600 px-2 md:px-3 py-1.5 rounded text-[10px] md:text-xs transition-colors" onClick={()=>call('/reset-all')}>XÓA HẾT</button>
                </div>}
              </div>
              ...
              {isHost && <button className="w-full mt-4 bg-yellow-600 hover:bg-yellow-500 text-black font-black p-4 rounded-xl shadow-lg transition-transform active:scale-95" onClick={()=>call('/phase-strategy')}>CHỐT SỔ →</button>}
            </div>
          </div>
          
          <div className="bg-slate-900 p-5 md:p-8 rounded-2xl border border-slate-800 shadow-2xl">
            <h2 className="text-sm font-bold text-yellow-500 mb-4 uppercase tracking-widest border-b border-slate-800 pb-3 text-center md:text-left">📖 Tàng Kinh Các (Chỉ số & Luật chơi)</h2>
            {statsWiki ? (
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6 text-xs md:text-sm text-slate-400">
                <div className="space-y-2 bg-slate-800/30 p-4 rounded-xl border border-slate-800/50">
                  <h3 className="text-blue-400 font-black mb-3 text-sm">VŨ KHÍ (HP: 300)</h3>
                ...
                <div className="space-y-2 bg-slate-800/30 p-4 rounded-xl border border-slate-800/50">
                  <h3 className="text-green-400 font-black mb-3 text-sm">KHIÊN GIÁP</h3>
                ...
                {/* --- CẬP NHẬT CỘT 3: CƠ CHẾ CHIẾN THUẬT RÕ RÀNG --- */}
                <div className="space-y-3 bg-slate-800/30 p-4 rounded-xl border border-slate-800/50 text-white font-mono text-[11px] md:text-xs">
                  <h3 className="text-purple-400 font-black mb-2 font-sans text-sm">CƠ CHẾ & CHIẾN THUẬT AI</h3>
                  <p>🏃 <span className="text-slate-400">Tốc độ =</span> 160 - (Nặng VK + Khiên)</p>
                  <p>💥 <span className="text-slate-400">Khắc hệ =</span> <span className="text-yellow-400 font-bold">x2.0 Sát thương!</span></p>
                  <p>🏹 <span className="text-slate-400">Hit & Run:</span> AI tự lùi nếu địch tiến vào Tầm Tối Thiểu.</p>
                  <p>🌿 <span className="text-slate-400">Núp Lùm:</span> Quét Radar, lách góc né tường trốn giao tranh.</p>
                  <p>🔥 <span className="text-red-400 font-bold">Berserk Mode:</span> Máu rớt dưới 30% (90 HP) tự động HỦY NÚP, hóa điên lao lên tử chiến lật kèo!</p>
                </div>
              </div>
      ...
      {state.status === 'strategy' && (
        <div className="max-w-2xl w-full bg-slate-900 p-5 md:p-10 rounded-3xl border-2 border-purple-500/50 shadow-2xl">
          <div className="text-center mb-6">
            <h2 className="text-[10px] font-bold text-purple-400 uppercase tracking-widest mb-2">ĐẾM NGƯỢC CHIẾN THUẬT</h2>
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <select className="w-full bg-slate-800 p-4 rounded-xl text-xs md:text-sm outline-none" value={strat.target_rule} onChange={e=>setStrat({...strat, target_rule:e.target.value})}>
                <option value="closest">Đánh Gần nhất</option><option value="lowest_hp">Đánh Yếu máu nhất</option><option value="highest_hp">Đánh Trâu nhất</option><option value="counter">Đánh Khắc hệ</option>
              </select>
              <select className="w-full bg-slate-800 p-4 rounded-xl text-xs md:text-sm outline-none" value={strat.camp_until} onChange={e=>setStrat({...strat, camp_until:parseInt(e.target.value)})}>
                <option value="99">Đánh luôn từ đầu</option><option value="5">Núp né tường tới Top 5</option><option value="3">Núp tới Top 3</option><option value="2">Núp tới Chung Kết</option>
              </select>
            </div>
            <button className="w-full mt-4 bg-purple-600 hover:bg-purple-500 font-black text-white p-5 rounded-2xl shadow-xl transition-all active:scale-95" onClick={()=>call('/update-strategy', strat).then(d=> alert(d.status==='success' ? "ĐÃ KHÓA CHIẾN THUẬT!" : d.message))}>KHÓA CHIẾN THUẬT BÍ MẬT</button>
          </div>
        </div>
      )}
        ...
        <div className="flex flex-col lg:flex-row gap-4 w-full max-w-7xl justify-center items-center lg:items-start">
          <div className="w-full lg:max-w-max flex flex-col gap-4 relative">
            <div className="relative bg-black rounded-2xl md:rounded-3xl border-4 md:border-[6px] border-slate-800 shadow-2xl overflow-hidden w-full">
              ...
              {state.status === 'finished' && (
                <div className="absolute inset-0 bg-slate-950/90 flex flex-col items-center justify-center p-6 text-center animate-in fade-in duration-500">
                  <h2 className="text-4xl md:text-7xl font-black text-yellow-500 mb-2 italic tracking-tighter">THE CHAMPION</h2>
                  ...
                  {isHost && (
                    <div className="mt-8 flex flex-col sm:flex-row gap-3">
                        <button className="bg-blue-600 hover:bg-blue-500 px-8 py-3 rounded-full font-black text-white shadow-xl" onClick={()=>call('/rematch')}>TÁI ĐẤU (Giữ Team)</button>
                        <button className="bg-red-900 hover:bg-red-700 px-6 py-3 rounded-full font-bold text-xs shadow-xl" onClick={()=>call('/reset-all')}>XÓA ĐÁNH LẠI</button>
                    </div>
                  )}
                </div>
              )}
            ...
            {state.status === 'finished' && isHost && (
              <div className="bg-slate-900 p-4 md:p-6 rounded-2xl border border-slate-800 w-full shadow-2xl animate-in slide-in-from-bottom-5">
                <h3 className="font-bold text-yellow-500 mb-2 uppercase text-xs">🎙️ XUẤT LỊCH SỬ CHO BÌNH LUẬN VIÊN AI</h3>
                <textarea readOnly className="w-full bg-black text-slate-300 p-3 rounded text-[10px] h-32 outline-none font-mono custom-scrollbar border border-slate-800" value={promptText}></textarea>
                <button className="mt-2 w-full bg-slate-700 hover:bg-slate-600 text-xs py-3 rounded font-bold transition-colors" onClick={()=>{navigator.clipboard.writeText(promptText); alert("Đã Copy Báo Cáo! Ném vào ChatGPT hoặc Gemini nhé.");}}>📋 COPY BÁO CÁO</button>
              </div>
            )}
          </div>
          ...
          <div className="bg-slate-900 p-4 md:p-6 rounded-2xl w-full lg:max-w-xs flex flex-col border border-slate-800 shadow-xl h-[250px] lg:h-auto" style={{ lg: { height: state.config.h + 12 } }}>
             ...
             <div className="flex-grow overflow-auto space-y-2 flex flex-col-reverse font-mono text-[10px] md:text-[11px] leading-relaxed pr-1 custom-scrollbar">
                {[...(state.logs || [])].reverse().map((l, i) => <div key={i} className={`p-2 rounded-lg border-l-4 ${l.includes('💀') || l.includes('🏆') || l.includes('🔥') ? 'bg-red-950/40 border-red-600 text-red-200' : 'bg-slate-800/80 border-slate-600 text-slate-300'}`}>{l}</div>)}
             </div>
          </div>
        </div>
      )}
